...
outlives any one specification revision). Inspection still has no
canonical table yet (Sprint 16) — see GoldenThreadOut's not_yet_available
list, the honest, explicit stand-in rather than silently omitting it.

The per-component composition (specs/evidence/changes/responsible party/
external references) lives in app/development/composition.py — Property
360 (Sprint 13) needed the exact same bundle, just gathered by property
instead of by building, so it was extracted there rather than copied.
"""
...
NOT_YET_AVAILABLE = ["inspections (Sprint 16)"]
